...
// xl 新增方法
+ (NSArray *)xl_backtraceOfNSThread:(NSThread *)thread {
    return _xl_MethodNamesOfThread(bs_machThreadFromNSThread(thread));
}

+ (NSArray *)xl_backtraceOfCurrentThread {
    return [self xl_backtraceOfNSThread:[NSThread currentThread]];
}

+ (NSArray *)xl_backtraceOfMainThread {
    return [self xl_backtraceOfNSThread:[NSThread mainThread]];
}
...
NSArray *_xl_MethodNamesOfThread(thread_t thread) {
    uintptr_t backtraceBuffer[50];
    int i = 0;
    ...
    bs_symbolicate(backtraceBuffer, symbolicated, backtraceLength, 0);
    for (int i = 0; i < backtraceLength; ++i) {
        NSString *methodName = xl_methodName(i, backtraceBuffer[i], &symbolicated[i]);
...
NSString* xl_methodName(const int entryNum,
                               const uintptr_t address,
                               const Dl_info* const dlInfo) {
    char faddrBuff[20];
    char saddrBuff[20];
